...
def _render_subitem_frag(aud_id: int, pratica_num: int, pratica_nome: str,
    ...
    with col_dir:
        ...
        try:
            ...
            _crit_main = _gc_crit(pratica_num, sub_idx)
            _niveis_main = _crit_main.get('niveis', {})
            _ev_main     = _crit_main.get('evidencias_exigidas', '') or ''
            _re_main     = _crit_main.get('regras_especiais', '') or ''
            if _niveis_main or _ev_main:
                with st.expander("📖 Critérios (PO.AUT.002)", expanded=False):
                    # ── Sub-tabs: Níveis | Evidência ──
                    _tab_nv, _tab_ev = st.tabs(["📊 Critérios por nível 0–4", "📋 Evidência exigida"])
                    ...
                    with _tab_nv:
                        ...
                        if _rows_nv_main:
                            ...
                            st.dataframe(
                                _pd_nv.DataFrame(_rows_nv_main),
                        ...
                        if _re_main:
                            st.warning(f"⚠️ **Regras Especiais:** {_re_main}")
                        with st.expander("📏 Regras gerais (todas as práticas)", expanded=False):
                            st.caption(_RG_crit)
                        ...
                        if _ev_main:
                            ...
                            for _ep in _ev_main.split(';'):
                                for _eq in _ep.splitlines():
                                    _eq = _eq.strip(' -\t')
                                    if _eq:
                                        _ev_itens.append(_eq)
                            if not _ev_itens:
                                _ev_itens = [_ev_main.strip()]
                            import pandas as _pd_ev
                            _df_ev = _pd_ev.DataFrame(
                                [{"#": i+1, "Evidência exigida": txt} for i, txt in enumerate(_ev_itens)]
                            )
                            st.dataframe(
                                _df_ev,
                                use_container_width=True,
                                ...
                                column_config={
                                    ...
                                    "Evidência exigida": st.column_config.TextColumn(width="large"),
                                }
                            )
                        else:
                            # Fallback: evidência da planilha
                            if ev_desc:
                                st.caption(ev_desc[:600])
                            else:
                                st.caption("Nenhuma evidência exigida registrada para este subitem.")
        except Exception:
            pass  # não quebra a UI
...
border-left:3px solid #6A1B9A;">
    ...
    {msg.get('conteudo', msg.get('content', ''))}{snap_info}
        ...
        # Pré-preencher: banco tem prioridade; NÃO usar IA como fallback
        # (se o usuário limpou o campo, deve permanecer vazio)
        default_desc = desc_nc_atual
        # ── Comentário automático baseado na decisão ──
        # Só auto-gerar comentário se: (a) comentário atual é vazio ou auto-gerado
        ...
        if st.button("💾 Salvar Decisão",key=f"{kp}_save",type="primary"):
            ...
            # Fechar este subitem (não reabrir) — usuário decide quando reabrir
            st.session_state[f'{kp}_open'] = False
            _limpar_cache_auditoria()
...
# ──────────────────────────────────────────────────────────────────────────────
    ...
    for pratica_num in sorted(int(x) for x in df_aval['pratica_num'].dropna().unique()):
        ...
        with st.expander(label, expanded=_prat_exp_open):
            st.session_state[_prat_key] = True  # mantém aberto nas reruns enquanto visível
            if not _prat_exp_open:
                # ── LAZY: prática fechada → mostrar apenas resumo, sem renderizar sub-itens ──
                _pend_str = f"⏳ {pend} pendente(s)" if pend > 0 else "✅ Concluída"
                _ia_str   = f" | 🤖 {ia_ok_p} IA" if ia_ok_p > 0 else ""
                st.caption(f"SA: {round(msa,1)} → Final: {round(mf,1) if mf is not None else '⏳'} | {_pend_str}{_ia_str} — Clique na prática para expandir sub-itens.")
            else:
                for _,av in df_p.iterrows():
                    ...
                    _nota_final_txt = f" → Final: {av.get('nota_final','⏳')}" if av.get('nota_final') is not None else ""
